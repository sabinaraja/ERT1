...
let expName = 'ERT1';  // from the Builder filename that created this script
let expInfo = {
...
flowScheduler.add(ecg_instructionsRoutineBegin());
flowScheduler.add(ecg_instructionsRoutineEachFrame());
flowScheduler.add(ecg_instructionsRoutineEnd());
flowScheduler.add(routine_3RoutineBegin());
flowScheduler.add(routine_3RoutineEachFrame());
flowScheduler.add(routine_3RoutineEnd());
flowScheduler.add(routine_2RoutineBegin());
flowScheduler.add(routine_2RoutineEachFrame());
flowScheduler.add(routine_2RoutineEnd());
flowScheduler.add(routine_1RoutineBegin());
flowScheduler.add(routine_1RoutineEachFrame());
flowScheduler.add(routine_1RoutineEnd());
flowScheduler.add(ecg_baselineRoutineBegin());
flowScheduler.add(ecg_baselineRoutineEachFrame());
flowScheduler.add(ecg_baselineRoutineEnd());
...
psychoJS.start({
  ...
  resources: [
    ...
    {'name': 'Face_Stimuli.xlsx', 'path': 'Face_Stimuli.xlsx'},
...
async function experimentInit() {
  ...
  ecg_instructionsClock = new util.Clock();
  text_9 = new visual.TextStim({
    win: psychoJS.window,
    name: 'text_9',
    text: 'We will now begin your ECG recording.\nPlease do not move or speak during this time. If you have any questions for your experimenter, ask now.\n\nThe recording will take 5 minutes, and the task will progress on its own once the 5 minutes are up.\n\nHit any key to continue. The ECG recording will begin now. ',
    font: 'Open Sans',
    units: undefined, 
    pos: [0, 0], height: 0.05,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
  ...
  ecg_baselineClock = new util.Clock();
...
var ecg_instructionsComponents;
function ecg_instructionsRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'ecg_instructions' ---
    t = 0;
    ecg_instructionsClock.reset(); // clock
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    psychoJS.experiment.addData('ecg_instructions.started', globalClock.getTime());
    key_resp_4.keys = undefined;
    key_resp_4.rt = undefined;
    _key_resp_4_allKeys = [];
    // keep track of which components have finished
    ecg_instructionsComponents = [];
    ecg_instructionsComponents.push(text_9);
    ecg_instructionsComponents.push(key_resp_4);
    
    ecg_instructionsComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function ecg_instructionsRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'ecg_instructions' ---
    // get current time
    t = ecg_instructionsClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
...
function ecg_instructionsRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('ecg_instructions.stopped', globalClock.getTime());
...
function routine_3RoutineBegin(snapshot) {
  return async function () {
    ...
    // keep track of which components have finished
    routine_3Components = [];
    routine_3Components.push(text_10);
...
function routine_2RoutineBegin(snapshot) {
  return async function () {
    ...
    // keep track of which components have finished
    routine_2Components = [];
    routine_2Components.push(text_11);
...
function routine_1RoutineBegin(snapshot) {
  return async function () {
    ...
    // keep track of which components have finished
    routine_1Components = [];
    routine_1Components.push(text_12);
...
var ecg_baselineComponents;
function ecg_baselineRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'ecg_baseline' ---
    t = 0;
    ecg_baselineClock.reset(); // clock
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    routineTimer.add(300.000000);
    // update component parameters for each repeat
    psychoJS.experiment.addData('ecg_baseline.started', globalClock.getTime());
    // keep track of which components have finished
    ecg_baselineComponents = [];
    ecg_baselineComponents.push(text_13);
    
    ecg_baselineComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function ecg_baselineRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'ecg_baseline' ---
    // get current time
    t = ecg_baselineClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
...
function ecg_baselineRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('ecg_baseline.stopped', globalClock.getTime());
    // Routines running outside a loop should always advance the datafile row
    if (currentLoop === psychoJS.experiment) {
      psychoJS.experiment.nextEntry(snapshot);
    }
...
function WelcomeRoutineBegin(snapshot) {
  return async function () {
    ...
    WelcomeComponents.push(key_resp_5);
...
function TrialFacesRoutineBegin(snapshot) {
  return async function () {
    ...
    FaceStimuli.setImage('face_trust_Face_Stimuli.xlsx');
    // keep track of which components have finished
